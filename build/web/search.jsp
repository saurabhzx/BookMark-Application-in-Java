...
<form action="search.jsp" method="POST" name="form" onsubmit="return submit_form();">
 ...
 if(request.getMethod().equalsIgnoreCase("POST"))
{
    ...
    for(int i = 0 ; i < item.length ; i++)
        {
            ResultSet rs2 = ob.executeQ("select * from tag_url where tag='"+item[i]+"'");
            while(rs2.next())
                {
                 %>
<%=rs2.getString("url")%>
